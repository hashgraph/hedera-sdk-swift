...
public final class TokenRevokeKycTransaction: Transaction {
  ...
  convenience init(_ proto: Proto_TransactionBody) {
    ...
    setAccountId(AccountId(proto.tokenRevokeKyc.account))
    setTokenId(TokenId(proto.tokenRevokeKyc.token))
  }
  ...
  override func getMethodDescriptor(_ index: Int) -> (_ request: Proto_Transaction, CallOptions?) ->
    ...
    >
  {
    nodes[circular: index].getToken().revokeKycFromTokenAccount
  }

  func build() -> Proto_TokenRevokeKycTransactionBody {
    var body = Proto_TokenRevokeKycTransactionBody()
