...
public final class TokenGrantKycTransaction: Transaction {
  ...
  convenience init(_ proto: Proto_TransactionBody) {
    ...
    setAccountId(AccountId(proto.tokenGrantKyc.account))
    setTokenId(TokenId(proto.tokenGrantKyc.token))
  }
  ...
  override func getMethodDescriptor(_ index: Int) -> (_ request: Proto_Transaction, CallOptions?) ->
    ...
    >
  {
    nodes[circular: index].getToken().grantKycToTokenAccount
  }

  func build() -> Proto_TokenGrantKycTransactionBody {
    var body = Proto_TokenGrantKycTransactionBody()
