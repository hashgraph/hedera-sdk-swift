...
internal final class TokenCreate: XCTestCase {
    ...
    internal func list<T>(amount: Int, element: T) -> [T] {
        guard amount > 0 else {
            return []
        }
        return Array(repeating: element, count: amount)
    }
    ...
    // Getting error: ACCOUNT_IS_TREASURY
    ...
    internal func testAdminKeySigFail() async throws {
        ...
        let admin_key = PrivateKey.generateEd25519()
        ...
        await assertThrowsHErrorAsync(
            try await TokenCreateTransaction()
                ...
                .symbol("F")
                .treasuryAccountId(account.id)
                .adminKey(.single(admin_key.publicKey))
                .expirationTime(.now + .minutes(5))
                .execute(testEnv.client)
                .getReceipt(testEnv.client),
                "expected error creating token"
    ...
    internal func testTooManyCustomFeesFail() async throws {
        ...
        let customFees: [AnyCustomFee] = list(amount: 11, element: .fixed(.init(amount: 10, feeCollectorAccountId: account.id)))
    ...
    internal func testTenFixedFees() async throws {
        ...
        let customFees: [AnyCustomFee] = list(amount: 10, element: .fixed(.init(amount: 10, feeCollectorAccountId: account.id)))
    ...
    internal func testTenFractionalFees() async throws {
        ...
        let customFees: [AnyCustomFee] = list(amount: 10, element: .fractional(.init(amount: "1/20", minimumAmount: 1, maximumAmount: 10, feeCollectorAccountId: account.id)))
